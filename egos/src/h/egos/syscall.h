#ifndef _EGOS_SYSCALL_H
#define _EGOS_SYSCALL_H
...
/* Process identifier.
 */
typedef unsigned int gpid_t;

/* A file is identified by the server that stores it and a file number
...
#define fid_val(s, i)		((fid_t) { .server = (s), .file_no = (i) })
#define fid_eq(f1, f2)		((f1).server == (f2).server && (f1).file_no == (f2).file_no)
...
enum grass_servers {
	...
	GPID_GATE,				// gate server
	GPID_DISK_PAGE,			// disk server for paging
	GPID_DISK_FS,			// disk server for file system
	GPID_FILE,				// default file server
	GPID_DIR,				// directory server (runs in user space)
	GPID_PWD,				// password server (runs in user space)

	/* Specific servers.
	 */
	GPID_FILE_RAM,			// ramfile server
	GPID_FILE_BLOCK,		// file server that uses the block server
	GPID_BLOCK,				// block server
	GPID_SYNC,				// sync server
...
int sys_recv(enum msg_type mtype, unsigned int max_time,
			void *msg, unsigned int size, gpid_t *src, unsigned int *uid);
