...
/* Tool for creating releases.  It looks for a set of keys specified
 * as arguments.  The keys work a little like #ifdef #else #endif
 * (although opposite).
 *
 * Code can be delimited by:
 *
 *		<<<<key
 *		... (removed if key included)
 *		<><>key   (optional)
 *		... (replacement)
 *		>>>>key
 *
 * For examples of use, see src/grass/process.c.
 */
