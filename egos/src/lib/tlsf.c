...
#define USE_GRASS		(1)
#define GRASS_ALIGN		(1 << 4)				// alignment = 16
#define ALMSK			(GRASS_ALIGN - 1)		// mask
#endif
...
#if USE_SBRK || USE_MMAP || USE_GRASS
...
static __inline__ void *get_new_area(size_t * size) 
{
	...
	if (!initialized) {
		...
		/* Align to 16 bytes.
		 */
		brk = (char *) (((address_t) brk + ALMSK) & ~ (address_t) ALMSK);
		initialized = 1;
	}
	area = brk;
	brk = (char *) brk + ((*size + ALMSK) & ~ALMSK);
	if (1) return area;
#endif // USE_GRASS
...
static __inline__ bhdr_t *process_area(void *area, size_t size)
{
    ...
    b = (bhdr_t *) GET_NEXT_BLOCK(ib->ptr.buffer, ib->size & TLSF_BLOCK_SIZE);
    b->size = ROUNDDOWN_SIZE(size - 3 * BHDR_OVERHEAD - (ib->size & TLSF_BLOCK_SIZE)) | USED_BLOCK | PREV_USED;
...
/******************************************************************/
    ...
    b = GET_NEXT_BLOCK(ib->ptr.buffer, ib->size & TLSF_BLOCK_SIZE);
...
/******************************************************************/
    ...
    b0 = GET_NEXT_BLOCK(ib0->ptr.buffer, ib0->size & TLSF_BLOCK_SIZE);
    lb0 = GET_NEXT_BLOCK(b0->ptr.buffer, b0->size & TLSF_BLOCK_SIZE);
    ...
    while (ptr) {
        ...
        b1 = GET_NEXT_BLOCK(ib1->ptr.buffer, ib1->size & TLSF_BLOCK_SIZE);
        lb1 = ptr->end;
        ...
        if ((unsigned long) ib1 == (unsigned long) lb0 + BHDR_OVERHEAD) {
            ...
            b0->size =
                ROUNDDOWN_SIZE((b0->size & TLSF_BLOCK_SIZE) +
                               (ib1->size & TLSF_BLOCK_SIZE) + 2 * BHDR_OVERHEAD) | USED_BLOCK | PREV_USED;
        ...
        if ((unsigned long) lb1->ptr.buffer == (unsigned long) ib0) {
            ...
            lb1->size =
                ROUNDDOWN_SIZE((b0->size & TLSF_BLOCK_SIZE) +
                               (ib0->size & TLSF_BLOCK_SIZE) + 2 * BHDR_OVERHEAD) | USED_BLOCK | (lb1->size & PREV_STATE);
            next_b = GET_NEXT_BLOCK(lb1->ptr.buffer, lb1->size & TLSF_BLOCK_SIZE);
            next_b->prev_hdr = lb1;
            b0 = lb1;
            ib0 = ib1;
    ...
    return (b0->size & TLSF_BLOCK_SIZE);
}
...
#if USE_MMAP || USE_SBRK || USE_GRASS
    if (!mp) {
        size_t area_size;
        void *area;
...
/******************************************************************/
	...
	TLSF_REMOVE_SIZE(tlsf, b);
        if (next_b->size & FREE_BLOCK) {
            ...
            next_b = GET_NEXT_BLOCK(next_b->ptr.buffer, next_b->size & TLSF_BLOCK_SIZE);
    ...
    cpsize = ((b->size & TLSF_BLOCK_SIZE) > new_size) ? new_size : (b->size & TLSF_BLOCK_SIZE);
...
void print_block(bhdr_t * b)
{
    if (!b)
        return;
    PRINT_MSG(">> [%p] (", b);
    if ((b->size & TLSF_BLOCK_SIZE))
        PRINT_MSG("%lu bytes, ", (unsigned long) (b->size & TLSF_BLOCK_SIZE));
    else
        PRINT_MSG("sentinel, ");
...
void print_all_blocks(tlsf_t * tlsf)
{
    ...
    while (ai) {
        ...
        while (next) {
            print_block(next);
            if ((next->size & TLSF_BLOCK_SIZE))
                next = GET_NEXT_BLOCK(next->ptr.buffer, next->size & TLSF_BLOCK_SIZE);
            else
                next = NULL;
        }
        ai = ai->next;
