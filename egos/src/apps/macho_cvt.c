...
static void usage(char *name){
	fprintf(stderr, "Usage: %s [-b base] macho-executable egos-executable\n", name);
	exit(1);
}
...
int main(int argc, char **argv){
	...
	for (i = 0; i < hdr.ncmds; i++) {
		...
		printf("%s: %llx %llx %llx %llx %x %d\n", argv[optind], u.sc.vmaddr, u.sc.vmsize, u.sc.fileoff, u.sc.filesize, u.sc.initprot, (int) outpos);
		...
		fseek(output, PAGESIZE + (u.sc.vmaddr - base), SEEK_SET);
		char buf[PAGESIZE];
		unsigned int total = u.sc.filesize, size;
		if ((u.sc.vmaddr - base) + total > max_size) {
			max_size = (u.sc.vmaddr - base) + total;
		}
		while (total > 0) {
