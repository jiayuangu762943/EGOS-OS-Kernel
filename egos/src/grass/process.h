...
struct process
{
...
#ifdef HW_MLFQ
	int number_clock_ticks; // total time running on CPU before dying;
	int priority_level;
	int quantum_time; // how much of a quantum left unused in clock ticks
	long start_time;
	long end_time;
	int number_yields;
#else
	...
	void (*intr_ip)();				// mostly used for debugging if anything
