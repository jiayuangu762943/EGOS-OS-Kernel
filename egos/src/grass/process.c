...
#define NUM_OF_LEVELS 3 // number of MLFQ levels
// TODO: is it ok to define them as constants or should we define them in an array
#define QUANTUM_0 10 // quantum 0 length
#define QUANTUM_1 20 // quantum 1 length
#define QUANTUM_2 30 // quantum 2 length
...
#ifdef HW_MLFQ
/*multi-level feedback queue data structure for scheduling*/
struct queue *mlfq[NUM_OF_LEVELS];
/*0th element holds the quantum length of 1st level,
1st element for the quantum length of 2nd level, etc*/
int quantum_length[NUM_OF_LEVELS];

void quantum_length_init()
{
    quantum_length[0] = 10;
    quantum_length[1] = 20;
    quantum_length[2] = 30;
...
struct process *proc_alloc(gpid_t owner, char *descr, unsigned int uid)
{
    ...
    p->uid = uid;
    p->owner = owner;
    // TODO: fix this -> HW_MEASURE
#ifdef HW_MEASURE
    p->number_clock_ticks = 0;
    p->start_time = sys_gettime();
    p->number_yields = 0;
#else
#endif
#ifdef HW_MLFQ
...
static void proc_to_runqueue(struct process *p)
{
    ...
    assert(p->priority_level < NUM_OF_LEVELS);
    queue_add(mlfq[p->priority_level], p);
#else
               ...
               enum msg_type mtype, const void *contents, unsigned int size)
{
    ...
    if (mtype == MSG_REPLY)
    {
        if (dst->state != PROC_WAITING || !mq->waiting || dst->server != src_pid)
        {
...
#ifdef HW_MEASURE
    long time_diff = sys_gettime() - proc->start_time;
    printf("process %u died after %u ticks, %u yields, %u milliseconds\n\r",
           proc->pid, proc->number_clock_ticks, proc->number_yields, time_diff);
#else
#endif
...
#ifdef HW_MLFQ
/*get the next process from the multi-level feedback queue*/
struct process *mlfq_get_next_proc()
{
    for (int i = 0; i < NUM_OF_LEVELS; ++i)
    {
        struct queue *q = mlfq[i];
        if (!queue_empty(q))
        {
            struct process *proc = queue_get(q);
            assert(proc->priority_level >= 0);
            assert(proc->priority_level < NUM_OF_LEVELS);
            proc->quantum_time = quantum_length[proc->priority_level];
            return proc;
        }
    }

    // there is no runnable process, current process continues with new quantum
    assert(proc_current->priority_level >= 0);
    assert(proc_current->priority_level < NUM_OF_LEVELS);
    proc_current->quantum_time = quantum_length[proc_current->priority_level];
    return NULL;
}
#else
#endif
...
void proc_yield(void)
{
    ...
    for (;;)
    {
         ...
         */
#ifdef HW_MLFQ
        while ((proc_next = mlfq_get_next_proc()) != 0)
#else
 ...
 */
void proc_pagefault(address_t virt, bool update)
{
    ...
    // printf("GOT PAGEFAULT: pid=%u addr=%p\n\r", p->pid, virt);
    ...
    if (update)
    {
        earth.tlb.map(index, abs_page, frame, P_READ | P_WRITE | P_EXEC);
    }
    else
    {
...
void proc_got_interrupt()
{
    ...
    case INTR_SYSCALL:
        ...
        break;
    case INTR_CLOCK:
        // do we use the Num of clock ticks to check if uses up all its quantum?
#ifdef HW_MEASURE
        proc_current->number_clock_ticks++;
        ema_update(es, proc_nrunnable);
#else
#endif

#ifdef HW_MLFQ
        if (proc_current->quantum_time > 0)
        {
            proc_current->quantum_time--;
            break;
        }
        if (proc_current->priority_level < NUM_OF_LEVELS - 1)
        {
            proc_current->priority_level++;
        }
...
void proc_dump(void)
{
    ...
    printf("%u processes (current = %u, nrunnable = %u, load = %.2f",
           proc_nprocs, proc_current->pid, proc_nrunnable, ema_avg(es));
#else
...
#ifdef HW_MLFQ
    printf("PID   DESCRIPTION  UID  PRIORITY  QUANTUM_TICKS_REMAINING  STATUS      RES SWP OWNER ALARM   EXEC\n\r");
#else
...
#ifdef HW_MLFQ
        printf("%4u: %-12.12s %3u 		%1u 		      %1u   ", p->pid, p->descr, p->uid, p->priority_level, p->quantum_time);
#else
...
#ifdef HW_MEASURE
    es = (struct ema_state *)malloc(sizeof(struct ema_state));
    ema_init(es, 0.01);
#else
#endif
